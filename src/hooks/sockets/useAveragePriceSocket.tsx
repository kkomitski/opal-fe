...
const useAveragePriceSocket = (symbol: string) => {
  const [message, setMessage] = useState<WebSocketResponseMessage | null>(null);
  const [error, setError] = useState<Event>();

  useEffect(() => {
    const socket = new WebSocket(`wss://stream.binance.com:9443/ws/${symbol}@avgPrice`);
...
export default useAveragePriceSocket;
